...
async function createUnfixedAccountPaybale() {
    const token = await authenticateUser();
...
describe('[E2E] - List all unfixed accounts payable by month', () => {
    let token;
    beforeEach(async () => {
        token = await authenticateUser();
        await createUnfixedAccountPaybale();
    });
    it('should be able to list all unfixed accounts by month', async () => {
        const response = await (0, supertest_1.default)(app_1.app)
            .get('/accounts-payable/unfixed/month')
            .set({ Authorization: `Bearer ${token}` })
            .query({ month: 1 });
        expect(response.status).toBe(200);
        expect(Array.isArray(response.body.unfixedAccountsPayable)).toBe(true);
        expect(response.body.unfixedAccountsPayable.length).toBe(2);
    });
});
//# sourceMappingURL=ListAllUnfixedAccountsByMonthController.e2e-spec.js.map