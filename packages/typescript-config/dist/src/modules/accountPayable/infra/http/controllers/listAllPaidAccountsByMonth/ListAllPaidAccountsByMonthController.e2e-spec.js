...
describe('[E2E] - List all paid accounts payable by month', () => {
    let token;
    beforeEach(async () => {
    ...
    it('should be able to list all paid accounts by month', async () => {
        const response = await (0, supertest_1.default)(app_1.app)
            .get('/accounts-payable/paid/month')
            .set({ Authorization: `Bearer ${token}` })
            .query({ month: 1 });
        expect(response.status).toBe(200);
        expect(Array.isArray(response.body.paidAccountsPayable)).toBe(true);
        expect(response.body.paidAccountsPayable.length).toBe(1);
        expect(response.body.paidAccountsPayableTotalAmount).toBe(1000);
    });
});
//# sourceMappingURL=ListAllPaidAccountsByMonthController.e2e-spec.js.map