...
async function createUnfixedAccountPaybale(financialCategoryId, token) {
    await (0, supertest_1.default)(app_1.app)
...
describe('[E2E] - List total spent by financial category to unfixed account payable', () => {
    let token;
    ...
    beforeEach(async () => {
        ...
        await createUnfixedAccountPaybale(financialCategoryId, token);
    });
    it('should be able to calculate the total spent by financial category to unfixed account payable', async () => {
        const response = await (0, supertest_1.default)(app_1.app)
            .get('/financial-category/total-spent/unfixed/account-payable')
            .set({ Authorization: `Bearer ${token}` })
            .query({ month });
...
//# sourceMappingURL=ListTotalSpentToUnfixedAccountPayableController.e2e-spec.js.map